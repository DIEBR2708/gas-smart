...
flowchart LR
  ...
  Verdict --> Panel[결과 패널: 순위·비용 분해·지도]
```
...
경로 순서로 정렬한 DP나 그리디로 풀 수 있다. 구현은
[itinerary.ts](../src/lib/domain/itinerary.ts)의 Gas Station 그리디다. 탱크와
예비량을 상태로 두고, 더 싼 곳이 앞에 있으면 지금 정류에서는 그곳까지 갈
만큼만 넣는다. 한 번에 가능하면 일정은 한 정류다.
...
- **운전 중 조작 유도**: 이 앱은 직접 길안내를 하지 않는다. 주유소 선택은 출발
  전에 끝내는 것을 전제로 하고, 실제 안내는 사용자가 쓰던 내비 앱에 맡긴다.
...
## 7. 프로토타입에 들어간 확장 (이전 "아직 하지 않은 것")

| 항목 | 구현 |
| --- | --- |
| 다회 주유 최적화 | 경로 순서 Gas Station 그리디. 탱크·예비량을 상태로 두고 방문 순서를 만든다. 두 곳 이상이면 `multi-stop` 판정 |
| 실주소·실시간 위치 입력 | `/api/places` + 지명 사전. 카카오 로컬 키가 있으면 키워드·주소·역지오코딩. 브라우저 Geolocation |
| 연비 학습 | 주유 기록은 `localStorage`에만 둔다. 두 건부터 최근 가중평균. 서버로 보내지 않음 |
| 할인 프로필 | 브랜드 조건부 정액·정률 규칙. 공통 카드 할인과 합성. 기기 안에만 저장 |
| 사용자 신고 | 폐업·휴업은 제외, 가격 불일치는 +40원/L. 계정 없이 이 브라우저에만 남김 |
| 정체 반영 | 출발 시각 기준 평일 출퇴근 휴리스틱. 카카오 키가 있고 출발이 10분~48시간 뒤면 미래운행정보 길찾기 |
| 오프라인 대비 | 마지막 계획을 `localStorage`에 두고, `/api/plan` 실패 시 캐시와 경고를 같이 보여 줌 |

서버 계정은 만들지 않았다. 위치·주유 기록·제보를 식별자와 묶지 않으므로
위치정보법상 궤적 결합을 피한다. 키 없이도 지명 사전과 합성 주유소로
위 기능이 끝까지 동작한다.
