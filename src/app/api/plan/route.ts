...
export async function POST(request: Request) {
  ...
  const route = getSampleRoute(body.routeId ?? "") ?? SAMPLE_ROUTES[0];
  ...
  const providers = resolveProviders(vehicle.fuelKind);

  try {
    const plan = await buildRefuelPlan(
      { route, vehicle, preferences, departAt },
      providers,
    );

    // 지도에 그릴 우회 구간 형상을 후보별로 붙여 보낸다.
