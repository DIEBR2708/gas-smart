...
- 지도 위(또는 차량·조건 탭)에서 **주소·지명 검색**으로 출발·도착을 정함.
  카카오 로컬이 꺼져 있으면 OpenStreetMap Nominatim으로 주소를 찾음
- **현재 위치** 또는 **지도에서 찍기**. 미리보기 iframe은 브라우저가 위치를
  막을 수 있어서, 그때는 지도를 눌러 지정하면 됨
- 샘플 경로(서울–대전 / 강남–인천공항 / 수원–강릉)로 한 번에 채울 수도 있음
- 차량 설정(유종, 실주행 연비, 탱크 용량, 현재 연료량, 예비량)
- 주유 기록으로 **실주행 연비 학습** (기기 안에만 저장, 두 건부터 추정)
- 주유 방식과 공통 카드 할인, **브랜드 조건부 할인 프로필**
- 출발 시각에 따른 정체(휴리스틱, 키가 있으면 카카오 미래운행정보)
- 시간의 가치, 최대 우회 거리·시간, 브랜드·셀프 필터
- 탱크가 짧으면 **나눠 넣는 일정**(다회 주유)
- 폐업·가격 불일치 **현장 제보** (다음 계산에 반영, 서버 계정 없음)
